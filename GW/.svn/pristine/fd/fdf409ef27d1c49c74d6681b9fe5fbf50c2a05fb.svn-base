...
- (void)layoutSubviews {
    ...
    //modify by wangqiwei
    NSUInteger page = ceilf(labelSize.width/maxWidth);
    labelSize.height =  labelSize.height*page;
    ...
    CGFloat yPos;
    CGFloat xPos;
    //modify by wangqiwei 1：处理toast专用
    if (self.OffsetFlag ==1)
    {
        yPos = (totalSize.height/2)+margin+yOffset;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect boxRect;
    if (self.OffsetFlag ==1)//1：处理toast专用
    {
         boxRect = CGRectMake(round((allRect.size.width - size.width) / 2) + self.xOffset,
                                   (size.height/2)+self.yOffset, size.width, size.height);
