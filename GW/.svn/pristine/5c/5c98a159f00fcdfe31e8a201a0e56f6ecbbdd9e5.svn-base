...
/*!
 @header VehicleDiagnosisInformViewController.m
 @abstract 车辆故障诊断消息类
 @author mengy
...
#import "App.h"
@interface VehicleDiagnosisInformViewController ()
{
...
@implementation VehicleDiagnosisInformViewController
...
- (void)dealloc
{
    ...
    [vehicleDiagnosisInformTableView release],vehicleDiagnosisInformTableView = nil;
...
//    titleLabel.text=[oRes getText:@"message.VehicleDiagnosisInformViewController.Title"];
//    titleLabel.font =[UIFont navBarTitleSize];
//    titleLabel.textColor=[UIColor whiteColor];
//    self.navigationItem.titleView=titleLabel;
    
    self.navigationItem.title=[oRes getText:@"message.VehicleDiagnosisInformViewController.Title"];
...
-(void)loadData
{
    ...
    self.vehicleDiagnosisInformMutableArray = [mCherryDBControl loadVehicleDiagnosisMessage:app.mUserData.mUserID];
    NSLog(@"%d",self.vehicleDiagnosisInformMutableArray.count);
    
    if (self.vehicleDiagnosisInformMutableArray.count==0) {
        //没有消息时编辑按钮不可用
        editBtn.enabled = NO;
    }
    else
    ...
    [vehicleDiagnosisInformTableView reloadData];
}
...
{
    Resources *oRes = [Resources getInstance];
    VehicleDiagnosisInformMessageData *tempData;
    //取到要删除文件的车友ID
    ...
    if (deleteMessagesNumber > 0) {
        ...
        //一次性删除多条消息
        [mCherryDBControl deleteVehicleDiagnosisMessageWithIDs:str];
...
- (void)tableViewEdit
{
    [vehicleDiagnosisInformTableView setEditing:!vehicleDiagnosisInformTableView.editing animated:YES];
}
...
- (void)viewDidUnload
{
    ...
    vehicleDiagnosisInformTableView= nil;
    deleteMutableArray=nil;
}
...
{
    return [self.vehicleDiagnosisInformMutableArray count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (cell==nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"VehicleDiagnosisInformCell" owner:self options:nil] lastObject];
    }
    ...
    NSLog(@"%d",[self.vehicleDiagnosisInformMutableArray count]);
    VehicleDiagnosisInformMessageData *mdata = [self.vehicleDiagnosisInformMutableArray objectAtIndex:row];
    //设置图标,通过读MessageInfo数据实现
    
    int typeCode = [mdata.mRepertType intValue];
    switch (typeCode) {
        case VEHICLE_DIAGNOSIS_TYPE_ONLINE:
            type.text = [oRes getText:@"message.VehicleDiagnosisInformViewController.online"];
            break;
        case VEHICLE_DIAGNOSIS_TYPE_PERIODIC:
            type.text = [oRes getText:@"message.VehicleDiagnosisInformViewController.periodic"];
            break;
        case VEHICLE_DIAGNOSIS_TYPE_AUTO:
            type.text = [oRes getText:@"message.VehicleDiagnosisInformViewController.auto"];
            break;
        default:
            break;
    }
    ...
    switch ([mdata.mCheckResult intValue]) {
        case VEHICLE_DIAGNOSIS_STATE_SUCCESS:
            result = [oRes getText:@"message.VehicleDiagnosisInformViewController.success"];
            statusImage.image = [UIImage imageNamed:@"message_vehicleControl_success"];
            break;
        case VEHICLE_DIAGNOSIS_STATE_SUCCESS_FAULT:
            result = [oRes getText:@"message.VehicleDiagnosisInformViewController.successFault"];
            statusImage.image = [UIImage imageNamed:@"message_vehicleControl_failure"];
            break;
        case VEHICLE_DIAGNOSIS_STATE_FAILURE:
            result = [oRes getText:@"message.VehicleDiagnosisInformViewController.successFailure"];
            statusImage.image = [UIImage imageNamed:@"message_vehicleDiagnosis_failure"];
            break;
        default:
            break;
    }
    message.text = [NSString stringWithFormat:@"%@%@%@%@",[[App getInstance] searchCarNum:mdata.mMessageKeyID],[oRes getText:@"message.VehicleDiagnosisInformViewController.message1"],[App getDateWithTimeSince1970:mdata.mCheckTime],result];
...
{
    if (indexPath.row==[self.vehicleDiagnosisInformMutableArray count])
    {
        return NO;
    }
    else
...
forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [deleteMutableArray addObject:[self.vehicleDiagnosisInformMutableArray objectAtIndex:indexPath.row]];
    [self.vehicleDiagnosisInformMutableArray removeObjectAtIndex:indexPath.row];
    NSArray *indexPaths = [NSArray arrayWithObject:indexPath];
    //    删除效果，暂不启用
    [vehicleDiagnosisInformTableView deleteRowsAtIndexPaths:indexPaths withRowAnimation:UITableViewRowAnimationFade];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSUInteger row = [indexPath row];
    if(row<_vehicleDiagnosisInformMutableArray.count)
    {
        VehicleDiagnosisInformMessageData *rowValue = [_vehicleDiagnosisInformMutableArray objectAtIndex:row];
        VehicleDiagnosisInformDetailViewController *vehicleDiagnosisInformDetailVC=[[VehicleDiagnosisInformDetailViewController alloc]init];
        vehicleDiagnosisInformDetailVC.messageKeyID = rowValue.mMessageKeyID;
        [self.navigationController pushViewController:vehicleDiagnosisInformDetailVC animated:YES];
        [vehicleDiagnosisInformDetailVC release];
        App *app = [App getInstance];
