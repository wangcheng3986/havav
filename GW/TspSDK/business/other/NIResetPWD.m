...
- (void)createRequest:(NSString *)pwd loginName:(NSString *)loginName smsCode:(NSString *)smsCode
{
    ...
    mRequest.fName = [NSString stringWithFormat:@"GW.M.UPDATE_PASSWORD_SERVICE"];
...
/*!
 ...
 @result 无
 */
- (void)sendRequestWithAsync:(id<NIResetPWDDelegate>)delegate
{
    mDelegate = delegate;
...
- (void)onFinish:(NSData*)data
{
    if(data)
    {
        ...
        if ([[responseResult objectForKey:@"error"]boolValue])
        {
            ...
            [mDelegate onResetPWDResult:nil code:RETURN_PROTOCOL_ERROR errorMsg:[self convertErrorCode:RETURN_PROTOCOL_ERROR]];
        }
        else
        {
            ...
            if ([header objectForKey:@"c"])
            {
                ...
                if(c==NAVINFO_RESULT_SUCCESS)
                {
                    ...
                    [mDelegate onResetPWDResult:nil code:mErrorCode errorMsg:[self convertErrorCode:mErrorCode]];
                }
                ...
                else if (c==NAVINFO_LOGIN_RESETPWD_RECOVERY_ACCOUNT_UNOPEND)
                {
                    mErrorCode = NAVINFO_LOGIN_RESETPWD_RECOVERY_ACCOUNT_UNOPEND;
                    [mDelegate onResetPWDResult:nil code:mErrorCode errorMsg:[self convertErrorCode:mErrorCode]];
                }
