/*!
 @header vehicleStatusNetManager.m
 @abstract 车辆监控网络管理类
 @author wangqiwei
...
@implementation VehicleStatusNetManager
/*!
...
- (void)createRequest:(NSString *)vin
{
    ...
    mRequest.fName = @"GW.M.SEND_VEHICLE_STATUS_REQUEST";
...
- (void)onFinish:(NSData*)data
{
    if(data)
    {
        ...
        if ([[responseResult objectForKey:@"error"]boolValue])
        {
            ...
            [mDelegate onVehicleStatusResult:nil code:RETURN_PROTOCOL_ERROR errorMsg:[self convertErrorCode:RETURN_PROTOCOL_ERROR]];
        }
        else
        {
            ...
            if ([header objectForKey:@"c"]) {
                ...
                if(c==NAVINFO_TOKENID_INVALID)
                {
                    ...
                    [mDelegate onVehicleStatusResult:nil code:mErrorCode errorMsg:[self convertErrorCode:mErrorCode]];
                }
                ...
                else if(c==NAVINFO_VEHICLE_STATUS_EXECUTING)
                {
                    mErrorCode = NAVINFO_VEHICLE_STATUS_EXECUTING;
                    [mDelegate onVehicleStatusResult:nil code:mErrorCode errorMsg:[self convertErrorCode:mErrorCode]];
                }
                else
                {
